 ...
 * @subpackage	com_contact
 */
class Jnt_HanhPhucModelServices_User extends JModelList
{
	...
	protected function getListQuery() {
		...
		$query = "SELECT s.*, c.id as cat_id, c.title as cat_title, c.alias AS category_alias 
...
// 			die ($db->getErrorMsg());
	...
	public function getUserInfo()
	{
		...
		// join over user params
		$query->select('param.metakey, param.metadesc');
		$query->join('LEFT', '#__user_params param ON p.user_id = param.user_id');
	...
	public function getCheckPromotionsAndWeddingImages()
	{
		...
		$arr['has_albums'] = ( !empty($result->id) && ($result->id > 0) ) ? true : false;
		...
		$arr['has_promotions'] = ( !empty($result->id) && ($result->id > 0) ) ? true : false;
