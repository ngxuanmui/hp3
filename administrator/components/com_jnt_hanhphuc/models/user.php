 ...
 * @since       1.6
 */
class Jnt_HanhphucModelUser extends JModelAdmin
{
	/**
	...
	*/
	...
	protected function loadFormData()
	{
		...
		if (!empty($params))
		{
			if (is_array($data))
			{
				$data['metakey'] =  $params->metakey;
				$data['metadesc'] =  $params->metadesc;
			}
			else 
			{
				$data->set('metakey', $params->metakey);
				$data->set('metadesc', $params->metadesc);
			}
	...
	protected function getUserParams()
	{
		...
		$query->select('*')->from('#__user_params')->where('user_id = ' . $id);
	...
	public function save($data)
	{
		...
		$query->select('id')->from($table)->where('user_id = ' . $pk);
		$db->setQuery($query);
		
		$checkUserParam = $db->loadResult();
		
		if ($checkUserParam)
		{
			$query->clear()
					->update($table)
					->set(array('metakey = ' . $db->quote($data['metakey']), 'metadesc = ' . $db->quote($data['metadesc'])))
					->where('user_id = ' . $user->id)
			;
		}
		else 
		{
			$values = array($user->id, $db->quote($data['metakey']), $db->quote($data['metadesc']));
			
			$query->clear()
					->insert($table)
					->columns(array('user_id', 'metakey', 'metadesc'))
